...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profiles } = await supabaseAdmin.from('user_profiles').select('user_id, location, role');
    const { data: logs } = await supabaseAdmin.from('activity_logs').select('user_id, total_co2e, emission_factors ( category )');
    ...
    const topCategory = Object.keys(categoryMap).sort((a, b) => categoryMap[b] - categoryMap[a])[0] || "None";
    const topLocation = Object.keys(locationMap).sort((a, b) => locationMap[b] - locationMap[a])[0] || "None";

    const regionText = Object.entries(locationMap)
      .sort(([, a], [, b]) => (b as number) - (a as number))
      .slice(0, 3)
      .map(([name, co2], idx) => `#${idx + 1} ${name}: ${Number(co2).toFixed(1)} kg CO2`)
    ...
    if (!aiResponse.ok) throw new Error(`Google Gemini rejected request: ${rawAiText}`);

    const aiData = JSON.parse(rawAiText);
    const generatedText = aiData.candidates?.[0]?.content?.parts?.[0]?.text;

    if (!generatedText) throw new Error("Gemini returned a 200 OK but body content was empty.");
    ...
    return new Response(
      JSON.stringify({ success: true, message: `Saved ${period_type} prescription to database.` }), 
